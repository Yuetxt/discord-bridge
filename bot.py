...
class BridgeBot(commands.Bot):
    ...
    def __init__(self, *, intents: discord.Intents):
        ...
        self.last_author: Dict[int, Dict] = {}  # channel_id -> {author_id, webhook_url, webhook_id, timestamp}
...
async def guildname(interaction: discord.Interaction, name: str = None):
    ...
    # Webhook username = "[{name}] {display_name}" — give the name up to 40 chars.
    MAX_LEN = 40
    if len(name) > MAX_LEN:
        await interaction.followup.send(
...
# =======================
# /bing: cross-server user search (recent speakers)
# =======================
...
async def unmute(interaction: discord.Interaction, user_id: str):
    """Unmute a user to allow their messages to be forwarded."""
    await interaction.response.defer(ephemeral=True)
...
async def listmuted(interaction: discord.Interaction):
    ...
    try:
        ...
        # Build response showing all muted users
...
async def poll(
    ...
    # Collect options (filter out None values)
    ...
    if duration_hours < 1 or duration_hours > 168:  # 1 week max
        await interaction.followup.send(
    ...
    try:
        ...
        # Calculate expiry time - UTC time + duration, keeping as naive datetime for storage
        ...
        # Get all channels in sync group
        channels = await bot.db.get_channels_in_group(group_uid)
        ...
        for channel_data in channels:
            try:
                ...
                # Send as bot message (not webhook) so buttons work
                poll_message = await target_channel.send(embed=embed, view=view)
                
                # Store poll message location
...
async def bing(interaction: discord.Interaction, query: str):
    channel = interaction.channel
    guild = interaction.guild  # current guild
...
async def forward_message(
    ...
    if not webhooks:
        ...
        if not webhook_url:
            ...
            return
        # Extract webhook_id from legacy URL for tracking
        ...
        should_reuse_webhook = (
            ...
            and (time.time() - last_author_info["timestamp"]) < 600  # Within 10 minutes
        )
        
        if should_reuse_webhook:
            # Reuse the same webhook for visual grouping in Discord
    ...
    # Resolve display name: use custom guild name if set, otherwise the real guild name.
    # Format: "[Server Name] Display Name"
    display_name = message.author.display_name
    ...
    # Download stickers as images (webhooks can't reliably forward stickers via sticker_ids)
        ...
        try:
            ...
            if original_channel is None:
                ...
                if original_message:
                    ...
                    # Clean and truncate preview - max 75 chars to leave room for "..."
                    ...
                    # Find synced message for the target channel
                    synced_messages = await bot.db.get_all_synced_for_message(
                        message.reference.message_id,
                        ...
                        buttons.append(
                            {
                                ...
                                "label": f"↩️ {author_name[:70]}",  # Truncate author name too
                                "disabled": True,
                            ...
                            buttons.append(
                                {
                                    "type": 2,
                                    "style": 5,
                                    "label": preview_trimmed,  # Already truncated with "..." if needed
                                    "url": discord_url,
                                }
                            )
                        else:
    ...
    payload = {
        ...
        "allowed_mentions": {
            "parse": ["users", "roles"]  # Excludes "everyone" to disable @everyone and @here
        }
    }
    if content:
    ...
    try:
        ...
        if response_data:
            ...
            # Store mapping for future replies/reactions/edits/deletes
